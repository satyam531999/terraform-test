...
3. GitHub Actions either deploys `dev` directly, or triggers Spinnaker if a webhook is configured.
...
- If `SPINNAKER_WEBHOOK_URL` is not configured, `.github/workflows/app-image.yml` continues using the Terraform-based `deploy_dev` fallback.
- If `SPINNAKER_WEBHOOK_URL` is configured as a GitHub secret, the same workflow skips `deploy_dev` and sends a webhook to Spinnaker instead.
- Webhook payload shape:
- `image`: full ECR image URI
- `imageTag`: semantic-version plus short SHA
- `environment`: `dev`
...
- Webhook trigger or Docker registry trigger
...
- Configure a Spinnaker webhook trigger using the payload described above
